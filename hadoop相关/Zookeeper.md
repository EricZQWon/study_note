...
- 应用：
    ...
    2. 分布式锁：利用 _EPHEMERAL_SEQUENTIAL_ 即自增排序的短暂节点实现分布式锁，规定序号值最小的节点拿到锁
    3. 同步协议: 可以用于实现两段提交协议。
...
- 会话状态：主要有这三个状态：connecting、connected、closed
    - 如果已经closed,那么不能恢复
    - connecting到connect是全双工的。属于可恢复的。

## 分布式锁
- 实现分布式锁的基础方法：
    1. 指定一个znode(root)作为“锁”
    2. 然后需要竞争的客户端在这个节点root下创建相应的子节点znode(EPHEMERAL_SEQUENTIAL，短暂顺序型的)。
    3. 使用getChildren方法获得root子节点中最小值的，使用Watcher回调通知最小值的客户端获得了锁
    4. 主动删除或使会话过期删除znode，则可以释放锁。
- 基础方法面临的困境：
    1. 羊群效应：即一次znode的创建、删除过程通知了太多暂时没资格竞争的节点，造成了服务器的压力。比如这时候最小值是2，在删除后，应该是3获得锁，但是可能一直通知到了100号
        - 解决方案：采用链式的通知顺序，即在删除节点时才通知下一个顺序的客户端。比如2删除后仅需要通知序号为3的客户端
    2. 可恢复的异常：前面提到的比如因连接丢失而不了解作为“锁”的那个root节点是否真的被创建成功了（**分布式的部分失败困境**）。并且这个操作是一个非幂等性的操作，如果第一次已经成功了却依然重试可能会导致一个孤儿节点（因为是顺序性的创建），甚至是导致死锁。
        - 解决方案：在创建znode的时候应该给它一个独一无二的序号来判断是否是已经创建成功，比如会话的ID就是一个不错的选择。命名格式如 lock-\<sessionId>\-\<squence number>。这样可以唯一确定一个结点。
    3. 不可恢复的异常：比如会话已经过期，那么因为是创建的短暂型znode，那么都会被删除。这时候就应该释放锁，但是这个时候很难让客户端意识到它不应该再持有锁，而应该重新创建并尝试申请获取锁。
        - 解决方案：ZooKeeper本身实现，但书上说的WriteLock笔者暂时只找到了JUC包下的。 
